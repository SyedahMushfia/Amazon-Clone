...
//This context is only for the imageInfo prop.

export const ImageStylingContext = createContext<ImageInfoProps | undefined>(
  undefined
);
...
export function useImageStylingContext() {
  const images = useContext(ImageStylingContext);

  if (images === undefined) {
    throw new Error(
      "useImageStylingContext must be used with ImageStylingContext"
    );
  }
